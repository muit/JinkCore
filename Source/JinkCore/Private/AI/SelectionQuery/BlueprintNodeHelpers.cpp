...
#include "AI/SelectionQuery/BlackboardData.h"
...
#undef GET_STRUCT_NAME_CHECKED
	...
	bool FindNodeOwner(AActor* OwningActor, UBTNode* Node, USelectionQueryComponent*& OwningComp, int32& OwningInstanceIdx)
	{
		bool bFound = false;
		...
		{
			TInlineComponentArray<USelectionQueryComponent*> Components;
			OwningActor->GetComponents(Components);
			for (int32 ComponentIndex = 0; ComponentIndex < Components.Num(); ComponentIndex++)
			{
				USelectionQueryComponent* BTComp = Components[ComponentIndex];
				if (BTComp)
				{
