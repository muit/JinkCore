...
#include "SelectionQuery/Tasks/BTTask_RunBehavior.h"
#include "SelectionQuery/BlackboardComponent.h"
#include "SelectionQuery/SelectionQueryManager.h"
#if WITH_EDITOR
#include "Kismet2/KismetEditorUtilities.h"
#endif // WITH_EDITOR

DEFINE_STAT(STAT_AI_SelectionQuery_Tick);
DEFINE_STAT(STAT_AI_SelectionQuery_LoadTime);
DEFINE_STAT(STAT_AI_SelectionQuery_SearchTime);
DEFINE_STAT(STAT_AI_SelectionQuery_ExecutionTime);
DEFINE_STAT(STAT_AI_SelectionQuery_AuxUpdateTime);
DEFINE_STAT(STAT_AI_SelectionQuery_NumTemplates);
DEFINE_STAT(STAT_AI_SelectionQuery_NumInstances);
DEFINE_STAT(STAT_AI_SelectionQuery_InstanceMemory);
...
{
	SET_DWORD_STAT(STAT_AI_SelectionQuery_NumTemplates, 0);
...
int32 USelectionQueryManager::GetAlignedDataSize(int32 Size)
{
	// round to 4 bytes
	return ((Size + 3) & ~3);
}
...
struct FNodeInitializationData
{
	...
	{
		SpecialDataSize = USelectionQueryManager::GetAlignedDataSize(SpecialNodeMemory);

		const uint16 NodeMemorySize = NodeMemory + SpecialDataSize;
		DataSize = (NodeMemorySize <= 2) ? NodeMemorySize : USelectionQueryManager::GetAlignedDataSize(NodeMemorySize);
	}
...
static void InitializeNodeHelper(UBTCompositeNode* ParentNode, UBTNode* NodeOb,
	...
	USelectionQuery& TreeAsset, UObject* NodeOuter)
{
	...
	if (CompositeOb)
	{
		for (int32 ServiceIndex = 0; ServiceIndex < CompositeOb->Services.Num(); ServiceIndex++)
		{
			...
			{
				UE_LOG(LogSelectionQuery, Warning, TEXT("%s has missing service node! (parent: %s)"),
					*TreeAsset.GetName(), *USelectionQueryTypes::DescribeNodeHelper(CompositeOb));
		...
		for (int32 ChildIndex = 0; ChildIndex < CompositeOb->Children.Num(); ChildIndex++)
		{
			...
			for (int32 DecoratorIndex = 0; DecoratorIndex < ChildInfo.Decorators.Num(); DecoratorIndex++)
			{
				...
				{
					UE_LOG(LogSelectionQuery, Warning, TEXT("%s has missing decorator node! (parent: %s, branch: %d)"),
						*TreeAsset.GetName(), *USelectionQueryTypes::DescribeNodeHelper(CompositeOb), ChildIndex);
			...
			{
				USelectionQuery& SubtreeAsset = *SubtreeTask->GetSubtreeAsset();
				...
				for (int32 DecoratorIndex = 0; DecoratorIndex < SubtreeAsset.RootDecorators.Num(); DecoratorIndex++)
				{
					...
					{
						UE_LOG(LogSelectionQuery, Warning, TEXT("%s can't inject decorator from %s! (parent: %s, branch: %d)"),
							*TreeAsset.GetName(), *SubtreeAsset.GetName(),
							*USelectionQueryTypes::DescribeNodeHelper(CompositeOb), ChildIndex);
						continue;
					}
			...
			else if (ChildInfo.ChildTask)
			{
				...
				for (int32 ServiceIndex = 0; ServiceIndex < ChildInfo.ChildTask->Services.Num(); ServiceIndex++)
				{
					...
					{
						UE_LOG(LogSelectionQuery, Warning, TEXT("%s has missing service node! (parent: %s)"),
							*TreeAsset.GetName(), *USelectionQueryTypes::DescribeNodeHelper(ChildInfo.ChildTask));
...
bool USelectionQueryManager::LoadTree(USelectionQuery& Asset, UBTCompositeNode*& Root, uint16& InstanceMemorySize)
{
	SCOPE_CYCLE_COUNTER(STAT_AI_SelectionQuery_LoadTime);

	for (int32 TemplateIndex = 0; TemplateIndex < LoadedTemplates.Num(); TemplateIndex++)
	{
		FSelectionQueryTemplateInfo& TemplateInfo = LoadedTemplates[TemplateIndex];
	...
	if (Asset.RootNode)
	{
		FSelectionQueryTemplateInfo TemplateInfo;
		...
		INC_DWORD_STAT(STAT_AI_SelectionQuery_NumTemplates);
...
void USelectionQueryManager::InitializeMemoryHelper(const TArray<UBTDecorator*>& Nodes, TArray<uint16>& MemoryOffsets, int32& MemorySize, bool bForceInstancing)
{
	TArray<FNodeInitializationData> InitList;
...
struct FNodeClassCounter
{
	...
	void Print(const TCHAR* Separator=TEXT(" "))
	{
		...
		{
			UE_LOG(LogSelectionQuery, Display, TEXT("%s%s%s(%s)%s%d")
				, Separator
...
void USelectionQueryManager::DumpUsageStats() const
{
	...
	UE_LOG(LogSelectionQuery, Display, TEXT("----------------------USelectionQueryManager::DumpUsageStats----------------------\nBehavior Trees:"));

	// get all BTNode classes
	
	for (TObjectIterator<USelectionQuery> It; It; ++It)
	{
		FNodeClassCounter TreeNodeCounter;
		UE_LOG(LogSelectionQuery, Display, TEXT("--- %s ---"), *(It->GetName()));
		StatNodeUsage(It->RootNode, TreeNodeCounter);
		TreeNodeCounter.Print();
		AllNodesCounter.Append(TreeNodeCounter);
	}
	
	UE_LOG(LogSelectionQuery, Display, TEXT("--- Total Nodes usage:"));
	AllNodesCounter.Print(TEXT(","));
}

void USelectionQueryManager::AddActiveComponent(USelectionQueryComponent& Component)
{
	ActiveComponents.AddUnique(&Component);
}

void USelectionQueryManager::RemoveActiveComponent(USelectionQueryComponent& Component)
{
...
USelectionQueryManager* USelectionQueryManager::GetCurrent(UObject* WorldContextObject)
{
