...
#include "SQAuxiliaryNode.generated.h"

class USelectionQueryComponent;

struct FSQAuxiliaryMemory : public FSQInstancedNodeMemory
{
...
UCLASS(Abstract)
class AIMODULE_API USQAuxiliaryNode : public USQNode
{
	GENERATED_UCLASS_BODY()

	/** wrapper for node instancing: OnBecomeRelevant */
	void WrappedOnBecomeRelevant(USelectionQueryComponent& OwnerComp, uint8* NodeMemory) const;

	/** wrapper for node instancing: OnCeaseRelevant */
	void WrappedOnCeaseRelevant(USelectionQueryComponent& OwnerComp, uint8* NodeMemory) const;

	/** wrapper for node instancing: TickNode */
	void WrappedTickNode(USelectionQueryComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds) const;

	virtual void DescribeRuntimeValues(const USelectionQueryComponent& OwnerComp, uint8* NodeMemory, ESQDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const override;
...
protected:
	...
	virtual void OnBecomeRelevant(USelectionQueryComponent& OwnerComp, uint8* NodeMemory);

	/** called when auxiliary node becomes inactive
	 * this function should be considered as const (don't modify state of object) if node is not instanced! */
	virtual void OnCeaseRelevant(USelectionQueryComponent& OwnerComp, uint8* NodeMemory);

	/** tick function
	 * this function should be considered as const (don't modify state of object) if node is not instanced! */
	virtual void TickNode(USelectionQueryComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds);
...
FORCEINLINE uint8 USQAuxiliaryNode::GetChildIndex() const
{
	return ChildIndex;
}
